...
class MySQLDuplicateKeyIntegrationTest {
    ...
    fun startContainer() {
        assumeTrue(
            ...
            "Docker is not available; skipping MySQL tests",
        )
        container =
            MySQLContainer("mysql:9.3")
    ...
    @Test
    fun `duplicateKey should match primary key violation from MySQL`() {
        val ex =
        ...
        assertNotNull(ex)
        assertTrue(MySQLFilters.duplicateKey.matches(ex!!))
    }

    @Test
    fun `duplicateKey should match unique key violation from MySQL`() {
        val ex =
            dataSource.connection.use { conn ->
                conn.createStatement().use { stmt ->
                    stmt.execute("DROP TABLE IF EXISTS test")
    ...
    fun `duplicateKey should not match non unique constraint violation`() {
        ...
        assertNotNull(ex)
        assertFalse(MySQLFilters.duplicateKey.matches(ex!!))
    }
}
